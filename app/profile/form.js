...
export default function Form({ session }) {
    ...
    return (
        <div className={classes.main}>
            <select className={classes.select} onChange={handleSelect} value={selected}>
                {selectList.map((item) => (
            ...
            <form method="POST" action="/api/post/taste">
                {tasteList.map((item, idx) => (
                    <input className={classes.none} key={idx} name="taste" type="text" defaultValue={tasteList[0]} />
                ))}
                <input className={classes.none} name="email" type="text" defaultValue={session.user.email} />
